...
##交通手段一覧
mode = ["car","train","bus","cycle","walk"]

##旅行時間を辞書に格納
travel_dict = df_time.T.to_dict(orient="list")
...
def mode_choice(O,D,n=1):
    ##重み付き復元抽出
    return np.random.choice(mode,n,p=mode_selection_probability(O,D))
...
#O = 1
#D = 10
#result = [mode_choice(O,D)[0] for x in range(1000)]
##plt.hist(result)
